...
DO Q_loop= 1, no_discharges!15 
    ...
    DO j=1, jmax 
        ...
        waterlast=water
        water= waterM + (TR/2._dp+.0*sin(2._dp*pi*t/(12.4_dp*3600._dp*100._dp)))*sin(2._dp*pi*t/(12.4_dp*3600._dp))  !1.+ 1.*sin(2.*pi*j/500.) 
        ...
        DO iii=1, 1 !This can be used to try fancy time stepping techniques
            ...
            IF(variable_timestep) THEN
                ...
                !ELSEIF(.TRUE.) THEN
                    ...
                    DT1 = min(max(1.0e-02_dp*tmp2/max(tmp,1.0e-020_dp), 1.0_dp), 100.0_dp*3600.0_dp)
